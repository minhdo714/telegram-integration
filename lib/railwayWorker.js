...
async function fetchJSON(path, options = {}) {
    ...
    try {
        ...
        try {
            ...
            if (!response.ok) {
                // Return error in data structure instead of throwing
                return {
            ...
            return {
                error: `Invalid response from worker (${response.status})`,
                status: response.status,
                success: false,
...
export async function initiateQRLogin(userId = '1') {
    try {
...
// Initiate SMS login
export async function initiateSMSLogin(phoneNumber, sessionString = null, userId = '1') {
    try {
        return await fetchJSON('/api/sms-login/request-code', {
            ...
            body: JSON.stringify({ phoneNumber, sessionString, userId }),
        });
    } catch (error) {
...
// Verify SMS code
export async function verifySMSCode(phoneNumber, code, phoneHash, sessionString, userId = '1') {
    try {
        return await fetchJSON('/api/sms-login/verify-code', {
            ...
            body: JSON.stringify({ phoneNumber, code, phoneHash, sessionString, userId }),
        });
    } catch (error) {
...
// Verify 2FA password
export async function verify2FA(phoneNumber, password, sessionString = null, userId = '1') {
    try {
        return await fetchJSON('/api/sms-login/verify-password', {
            ...
            body: JSON.stringify({ phoneNumber, password, sessionString, userId }),
        });
    } catch (error) {
...
export async function botAction(action, body = null) {
    try {
        const method = (action === 'start' || action === 'stop' || body) ? 'POST' : 'GET';
        const options = { method };
        if (body) {
            options.headers = { 'Content-Type': 'application/json' };
            options.body = JSON.stringify(body);
        }
        return await fetchJSON(`/api/bot/${action}`, options);
    } catch (error) {
...
export async function getAccounts(userId) {
    try {
        const result = await fetchJSON(`/api/accounts/list?userId=${userId}`);
        if (result.error) {
            console.warn('Error fetching accounts:', result.error);
            // Return a safe empty structure instead of error
