...
def health():
    return jsonify({"status": "healthy", "service": "telegram-worker", "version": "1.4.0-debug-logs"}), 200
...
UPLOAD_FOLDER = '/tmp/uploads' if os.name != 'nt' else os.path.join(os.path.dirname(os.path.abspath(__file__)), 'uploads')
...
def upload_file():
    ...
    try:
        ...
        logger.info(f"Uploading {asset_type} for account {account_id}, filename: {file.filename}")
